...
/**
 * @file app/Handlers/Api/StatsApiHandler.php
 *
 ...
 * @ingroup api
 *
 * @brief Handler for managing statistics-related API endpoints.
 */
 ...
 * institusi, rata-rata sangia score, distribusi field, dll.
 */
class StatsApiHandler
{
    ...
    private function buildStats(): array
    {
        $totals = $this->db->fetchOne(
            'SELECT
                ...
                (SELECT COALESCE(AVG(sangia_score), 0) FROM researchers WHERE sangia_score > 0) AS avg_sangia_score,
        ...
        $fieldDistribution = $this->db->fetchAll(
            "SELECT
                ...
                AVG(sangia_score)        AS avg_score
             FROM researchers
             WHERE sangia_score > 0
             GROUP BY field
        ...
        $provinceDistribution = $this->db->fetchAll(
            'SELECT i.province,
                    ...
                    AVG(r.sangia_score) AS avg_impact
             FROM researchers r
        ...
        $yearlyTrend = $this->db->fetchAll(
            'SELECT publication_year AS year,
                    ...
                    COALESCE(AVG(sangia_score), 0) AS avg_score,
        ...
        return [
            ...
            'avg_sangia_score'     => round((float) ($totals['avg_sangia_score'] ?? 0), 2),
